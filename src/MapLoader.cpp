...
vector<string>& MapLoader::GetMapList(bool singlePlayer) {
	sp = singlePlayer;
	RefreshMapLists();
...
Map MapLoader::LoadMap(string name) {
	...
	mapFile.open(fullPath.c_str(), ios::in);
	if(mapFile.is_open()) {
		char block;
		string line;
		bool mapEnd = false;
		...
		while(getline(mapFile, line, '=')) {
			for(int i = 0; i < 5; i++) {
				if(line.compare(params[i]) == 0) {
					...
					switch(i) {		// set map parameter based on its string representation
						case 0:
